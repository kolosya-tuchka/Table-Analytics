...
csv_reader::csv_reader(const char* file_path, csv_config config)
{
...
void csv_reader::set_file_path(const char* file_path)
{
	if (input.is_open()) {
		return;
	}
...
void csv_reader::open(const char* file_path)
{
	set_file_path(file_path);
	open();
}
...
{
    return (input.peek() > 0) || cur_buf_size > cur_byte;
}
